...
def main():
    ...
    try:
        with sync_playwright() as pw:
            ...
            assert page.evaluate("document.querySelector('#step2 .quick-download').previousElementSibling.classList.contains('lead')")
            checks.append('Download action appears before optional account and folder settings')
            ...
            assert page.locator('#playPanel').is_hidden()
            assert page.locator('#buildOptions').get_attribute('open') is not None
            assert page.locator('#playCheckedBtn').is_disabled()
            assert page.locator('#confirmGameBtn').is_disabled()
            checks.append('Install choice shown first; play actions hidden before installation')
            ...
            assert page.locator('#buildOptions').get_attribute('open') is None
            assert page.locator('#jobPanel').evaluate("e=>e.compareDocumentPosition(document.querySelector('#step5')) & Node.DOCUMENT_POSITION_PRECEDING")
            ...
            page.evaluate('window.scrollTo(0,0)')
            page.screenshot(path=str(output/'12-ready-to-play.png'),full_page=False);artifacts.append('12-ready-to-play.png')
